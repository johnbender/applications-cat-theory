...
To facilitate the discussion it's useful to understand the two general forms that jQuery methods take when operating on the entire set of elements. The first iterates over the jQuery object-set and relies on side effects to communicate alterations of each constituent element back into the the parent jQuery object (See Appendix A for more). The second maps JavaScript functions over the \textbf{HTMLElement}s belonging to the jQuery object-set. Hereafter referred to as the map-form, this is the form we will address here. Figure \ref{fig:map-form} illustrates a simple example that uses the jQuery method \verb|map|.
...
The preservation of identity and composition is made obvious through simple illustration (Figure \ref{fig:id-and-compose}).
