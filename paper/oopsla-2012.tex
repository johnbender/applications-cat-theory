...
A short discussion of two important attributes required in the construction of jQuery methods will aid in understanding the forthcoming categories. The first is the context in which jQuery methods are expected to operate. That is, the value of \verb|this| within jQuery methods is the jQuery object-set, a set of DOM elements. The second is that all jQuery methods must reach into the jQuery object-set and extract the DOM elements to perform any meaningful work. This is generally done on either the first element or the whole set depending on the method. For the purposes of our work we are primarily concerned with those methods that opperate on the wholes set. Listing \ref{fig:chain-sample} illustrates an example of a method that exhibits these two attributes.

\begin{lstinputlisting}[
    language=JavaScript,
    caption={Sources of Html objects},
    label={fig:chain-sample}
]{js/while-form.js}
\end{lstlisting}

Listing \ref{fig:naive-add-class} shows a naive implementation of the \verb|addClass| method that meets these requirements. In it \verb|cls| is the name of a new CSS class to be added to the element and \verb|e| is each DOM element in the jQuery object-set.
